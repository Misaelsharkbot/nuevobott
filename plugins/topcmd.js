let handler = async (m, { conn, args }) => {
    let stats = Object.entries(global.DATABASE.data.stats).map(([key, value]) => {
...
└────`.trim()
    ...
    setTimeout(() => {
        if (global.DATABASE._data.chats[m.chat].deletemedia) conn.deleteMessage(m.chat, file.key)
    }, global.DATABASE._data.chats[m.chat].deletemediaTime)
}
