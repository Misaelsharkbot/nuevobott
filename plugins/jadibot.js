...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.connect().then(async ({user}) => {
      ...
      if (auth) return
      await parent.sendMessage(user.jid, `Puede iniciar sesión sin qr con el mensaje a continuación. Para obtener el código completo, envíe *${usedPrefix}getcode* para obtener el código exacto`, MessageType.extendedText)
  ...
  } else throw '¡No se pueden hacer bots dentro de bots!\n\nhttps://wa.me/' + global.conn.user.jid.split`@`[0] + '?text=.jadibot'
}
