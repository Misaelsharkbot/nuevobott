...
let handler = async (m, { conn }) => {
    ...
    if (new Date - user.lastweekly > 604800000) {
        conn.reply(m.chat, `Reclamastes y obtienes 20000 💵dinero y 3 🎁cajas legendarias`, m)
        user.money += 20000
        user.legendary += 3
        user.lastweekly= new Date * 1
    } else {
        let buttons = button(`Espere por favor *🕒${timers}* para poder reclamar de nuevo`, user)
