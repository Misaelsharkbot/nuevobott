...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (id in conn.tebakkabupaten) {
        conn.reply(m.chat, 'Todavía hay preguntas sin respuesta en este chat.', conn.tebakkabupaten[id][0])
        throw false
    }
...
`.trim()
    conn.tebakkabupaten[id] = [
        ...
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkabupaten[id]) await conn.sendButton(m.chat, `¡Se acabó el tiempo!\nLa respuesta era *${json.title}*`, 'MYTHIA×YUTA', 'Tebak Kabupaten', '.tebakkabupaten', conn.tebakkabupaten[id][0])
