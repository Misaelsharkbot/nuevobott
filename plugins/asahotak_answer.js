...
handler.before = async function (m) {
    ...
    if (!(id in this.asahotak)) return m.reply('El asunto ha terminado')
    if (m.quoted.id == this.asahotak[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*¡Correcto!* +${this.asahotak[id][2]} XP`, 'YUTABOTZ', 'Asah Otak', '.asahotak', m)
            clearTimeout(this.asahotak[id][3])
            delete this.asahotak[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*¡Un poco más!*`)
